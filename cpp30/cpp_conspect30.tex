...
\end{tabular}
\end{center}
\section{move-семантика в C++ , value categories}
В языке с++ мы с вами чего уже знаем про storage duration и че то там еще
У каждого выражения есть своя категория и есть иерархии категорий
expression -> rvalue ->xvalue , prvalue
|
glvalue -> lvalue , xvalue
Объект — это именованная или идентифицируемая область памяти, хранящая данные, а lvalue (locator value) — это выражение, ссылающееся на этот объект. Lvalue указывает на конкретное местоположение в памяти (адрес), что позволяет использовать его в левой части оператора присваивания. 
