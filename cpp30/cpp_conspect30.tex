...
% -------------------------------------------------------
\input{parts/01_fundamentals.tex}
\input{parts/02_oop.tex}
\input{parts/03_templates_raii.tex}
...
% -------------------------------------------------------
\input{parts/04_assoc_containers.tex}
...
% =============================================================
\section{Указатели на функции, методы, поля. Лямбды. Type erasure. \texttt{std::function}}
% =============================================================
...
\subsubsection{Что такое указатель на функцию}

Функция в~C++ занимает определённое место в~памяти (сегмент кода).
Указатель на функцию хранит \textbf{адрес точки входа} функции и позволяет
вызывать её косвенно --- через указатель.
...
int main() {
    ...
    op = add;          // имя функции неявно приводится к указателю
    std::cout << op(2, 3) << "\n";   // 5

    op = &mul;         // явное взятие адреса -- тоже допустимо
    std::cout << op(2, 3) << "\n";   // 6
}
\end{lstlisting}

\textbf{Синтаксис объявления:}

\begin{center}
\texttt{возвращаемый\_тип (*имя)(список\_параметров)}
\end{center}

Скобки вокруг \texttt{*имя} обязательны: без них получится объявление функции,
возвращающей указатель.

\subsubsection{Использование \texttt{typedef} и \texttt{using}}

Синтаксис указателей на функции громоздкий. Упрощаем:

\begin{lstlisting}
// C-style typedef
typedef int (*BinaryOp)(int, int);

// C++11 using (предпочтительно)
...
\subsubsection{Передача указателя на функцию как параметр}

\begin{lstlisting}
void apply_and_print(int a, int b, int (*func)(int, int)) {
    std::cout << func(a, b) << "\n";
}

// Или с using:
using BinaryOp = int(*)(int, int);
void apply_and_print(int a, int b, BinaryOp func) {
    std::cout << func(a, b) << "\n";
}

int main() {
    apply_and_print(10, 20, add);  // 30
    apply_and_print(10, 20, mul);  // 200
}
\end{lstlisting}

Это классический приём в~C --- callback-функции. Именно так работают
\texttt{qsort}, \texttt{signal} и~другие C-функции.

\subsubsection{Массив указателей на функции}

\begin{lstlisting}
int sub(int a, int b) { return a - b; }

int main() {
    BinaryOp ops[] = {add, mul, sub};
    for (auto op : ops) {
...
\subsubsection{Указатели на функции и шаблоны}

Шаблонная функция не является одной функцией --- это семейство.
Нужно указать конкретную инстанциацию:
...
int main() {
    int (*p)(int, int) = maximum<int>;     // OK: конкретная инстанциация
    // int (*q)(int, int) = maximum;       // Ошибка: какая именно?
    std::cout << p(3, 7) << "\n";          // 7
}
\end{lstlisting}

\subsubsection{Указатели на \texttt{noexcept}-функции (C++17)}

С C++17 \texttt{noexcept} является частью типа функции:

\begin{lstlisting}
void foo() noexcept {}
void bar() {}

void (*p1)() noexcept = foo;   // OK
void (*p2)()          = foo;   // OK: noexcept -> обычный (неявное приведение)
// void (*p3)() noexcept = bar; // Ошибка: обычный -> noexcept нельзя
\end{lstlisting}

% -------------------------------------------------------
\subsection{Указатели на методы (member function pointers)}
% -------------------------------------------------------

\subsubsection{Синтаксис}

Методы класса --- это \textbf{не обычные функции}. Они принимают скрытый
параметр \texttt{this}. Поэтому обычный указатель на функцию для них
\textbf{не подходит}:
...
int main() {
    // Тип: указатель на метод Calculator, принимающий (int,int), вернущий int
    int (Calculator::*method)(int, int) = &Calculator::add;

    Calculator calc;
    // Вызов через объект:
    std::cout << (calc.*method)(2, 3) << "\n";   // 5

    // Вызов через указатель на объект:
    Calculator* ptr = &calc;
    std::cout << (ptr->*method)(2, 3) << "\n";   // 5

    method = &Calculator::mul;
    std::cout << (calc.*method)(2, 3) << "\n";   // 6
}
\end{lstlisting}

\textbf{Важно:} оператор \texttt{\&} при взятии адреса метода \textbf{обязателен}
(в~отличие от обычных функций, где имя неявно конвертируется в~указатель).

\subsubsection{Синтаксис с \texttt{using}}

\begin{lstlisting}
using CalcMethod = int (Calculator::*)(int, int);
CalcMethod m = &Calculator::add;
\end{lstlisting}

\subsubsection{Указатели на \texttt{const}-методы}
...
int (Foo::*p)() const = &Foo::value;

const Foo f;
std::cout << (f.*p)() << "\n";  // 42
\end{lstlisting}

\subsubsection{Указатели на статические методы}

Статические методы \textbf{не имеют} \texttt{this}, поэтому для них используется
\textbf{обычный} указатель на функцию:
...
int (*p)(int) = &Utils::square;  // обычный указатель на функцию!
std::cout << p(5) << "\n";       // 25
\end{lstlisting}

\subsubsection{Использование \texttt{std::invoke} (C++17)}

Операторы \texttt{.*} и~\texttt{->*} неудобны. \texttt{std::invoke}
унифицирует вызов любого callable-объекта:

\begin{lstlisting}
#include <functional>

Calculator calc;
auto method = &Calculator::add;

// Вместо (calc.*method)(2, 3):
std::cout << std::invoke(method, calc, 2, 3) << "\n";  // 5

// Работает и с указателем на объект:
std::cout << std::invoke(method, &calc, 2, 3) << "\n"; // 5
\end{lstlisting}

% -------------------------------------------------------
\subsection{Указатели на поля (pointer to data member)}
% -------------------------------------------------------

\subsubsection{Синтаксис}

Можно взять <<адрес>> поля класса. На самом деле это не настоящий адрес,
а~\textbf{смещение} поля относительно начала объекта:
...
int main() {
    // Указатель на поле типа double класса Point
...
\subsubsection{Практическое применение: обобщённый доступ}
...
// Функция, сортирующая по любому полю:
template<typename T>
void sort_by(std::vector<Student>& v, T Student::*field) {
    std::sort(v.begin(), v.end(),
...
int main() {
    ...
    sort_by(students, &Student::age);   // по возрасту
    sort_by(students, &Student::gpa);   // по GPA
    sort_by(students, &Student::name);  // по имени
}
\end{lstlisting}

\subsubsection{Доступ через \texttt{std::invoke}}

\texttt{std::invoke} работает и с указателями на поля:

\begin{lstlisting}
Student s{"Alice", 20, 3.8};
auto ptr = &Student::name;
std::cout << std::invoke(ptr, s) << "\n";  // "Alice"
\end{lstlisting}
...
До C++11 для передачи <<кусочка логики>> в~алгоритм нужно было писать
отдельную функцию или функтор (класс с~\texttt{operator()}):
...
Это слишком многословно для простой проверки. Лямбды решают эту проблему.
...
\begin{center}
\texttt{[захват](параметры) спецификаторы -> тип\_возврата \{ тело \}}
\end{center}

\begin{itemize}
    \item \texttt{[захват]} --- какие переменные из окружающего контекста доступны внутри лямбды;
    \item \texttt{(параметры)} --- как у обычной функции;
    \item \texttt{спецификаторы} --- \texttt{mutable}, \texttt{noexcept}, \texttt{constexpr} и~др.;
    \item \texttt{-> тип\_возврата} --- можно опустить, если компилятор выведет сам;
    \item \texttt{\{ тело \}} --- код лямбды.
\end{itemize}

\subsubsection{Что такое лямбда на самом деле?}

Лямбда --- это \textbf{синтаксический сахар} для создания анонимного класса
с~\texttt{operator()}. Компилятор генерирует примерно следующее:
...
class __lambda_1 {
public:
    int operator()(int x) const {  // const по умолчанию!
        return x * 2;
    }
};
auto f = __lambda_1{};
...
Каждая лямбда имеет \textbf{уникальный тип}, даже если тело идентично:

\begin{lstlisting}
auto f1 = [](int x) { return x; };
auto f2 = [](int x) { return x; };
// decltype(f1) != decltype(f2) -- это разные типы!
\end{lstlisting}

\subsubsection{Захват переменных (capture)}

\paragraph{\texttt{[]} --- без захвата.} Лямбда не видит локальные переменные:

\begin{lstlisting}
int threshold = 10;
// auto f = [](int x) { return x > threshold; }; // Ошибка!
\end{lstlisting}

\paragraph{\texttt{[=]} --- захват всех по значению (копия).}

\begin{lstlisting}
int threshold = 10;
auto f = [=](int x) { return x > threshold; };
// threshold скопирован внутрь лямбды
threshold = 100;       // не повлияет на f
std::cout << f(50);    // true (сравнивается с 10)
\end{lstlisting}

\paragraph{\texttt{[\&]} --- захват всех по ссылке.}
...
\textbf{Осторожно:} захваченная ссылка может стать висячей!

\begin{lstlisting}
std::function<int()> make_counter() {
    int n = 0;
    return [&n]() { return ++n; };  // ОШИБКА: n уничтожен при выходе!
}
\end{lstlisting}

\paragraph{Поимённый захват.}

\begin{lstlisting}
int a = 1, b = 2, c = 3;
auto f = [a, &b](int x) {
    // a -- копия (read-only, т.к. operator() const)
    // b -- ссылка (можно менять)
    // c -- не захвачен, не доступен
    b += a + x;
};
\end{lstlisting}

\paragraph{Смешанный захват.}

\begin{lstlisting}
auto f = [=, &b]() { ... };  // все по значению, b по ссылке
auto g = [&, a]() { ... };   // все по ссылке, a по значению
\end{lstlisting}

\subsubsection{Захват \texttt{this}}

Внутри метода класса лямбда может захватить \texttt{this}:
...
public:
    ...
    auto make_safe_callback() {
        // [*this] (C++17) -- захват копии всего объекта
        return [*this]() {
            std::cout << "interval = " << interval_ << "\n";
        };
    }
...
\texttt{[this]} --- захватывает \textbf{указатель}. Если объект уничтожен, будет
dangling pointer.

\texttt{[*this]} (C++17) --- захватывает \textbf{копию} объекта. Безопасно,
но дорого для больших объектов.

\subsubsection{Init-capture (обобщённый захват, C++14)}

Позволяет создать новую переменную внутри захвата:

\begin{lstlisting}
// Переместить вектор в лямбду (а не копировать):
std::vector<int> data = {1, 2, 3, 4, 5};
auto f = [data = std::move(data)]() {
    for (int x : data) std::cout << x << " ";
};
// data здесь пуст -- он перемещён в лямбду

// Захват с переименованием:
int x = 42;
auto g = [value = x * 2]() { return value; };  // value == 84

// Захват unique_ptr (некопируемый тип):
auto ptr = std::make_unique<int>(42);
auto h = [p = std::move(ptr)]() { return *p; };
\end{lstlisting}

\subsubsection{Ключевое слово \texttt{mutable}}

По умолчанию \texttt{operator()} лямбды --- \texttt{const}. Чтобы изменять
захваченные по значению переменные, нужен \texttt{mutable}:

\begin{lstlisting}
int counter = 0;
auto inc = [counter]() mutable {
    return ++counter;  // OK: mutable позволяет менять копию
};
std::cout << inc() << "\n";  // 1
std::cout << inc() << "\n";  // 2
...
\begin{lstlisting}
auto bad = [counter]() {
    // return ++counter;  // Ошибка: operator() const
};
\end{lstlisting}

\subsubsection{Лямбда без захвата конвертируется в указатель на функцию}

Лямбда \textbf{без захвата} может быть неявно преобразована в~указатель
на~функцию с~подходящей сигнатурой:

\begin{lstlisting}
int (*fp)(int, int) = [](int a, int b) { return a + b; };
std::cout << fp(3, 4) << "\n";  // 7

// Это позволяет передавать лямбды в C API:
void c_api(void (*callback)(int)) { callback(42); }
c_api([](int x) { std::cout << x << "\n"; });  // OK
\end{lstlisting}

Лямбда \textbf{с~захватом} в~указатель на функцию \textbf{не конвертируется}:

\begin{lstlisting}
int offset = 10;
// int (*fp)(int) = [offset](int x) { return x + offset; }; // Ошибка!
\end{lstlisting}

\subsubsection{Generic-лямбды (C++14)}

Параметры лямбды можно объявить как \texttt{auto} --- компилятор сделает
шаблонный \texttt{operator()}:
...
C++20 позволяет явно указать шаблонные параметры:

\begin{lstlisting}
// Ограничение: только целочисленные типы
auto sum = []<typename T>(const std::vector<T>& v) requires std::integral<T> {
    T result = 0;
...
// sum(vd);  // Ошибка: double не integral
\end{lstlisting>

\subsubsection{Рекурсивные лямбды}

Лямбда не может вызвать саму себя напрямую, потому что \texttt{auto}
ещё не выведен в~момент определения тела. Решения:

\begin{lstlisting}
// 1. Через std::function (с накладными расходами):
std::function<int(int)> factorial = [&factorial](int n) -> int {
    return n <= 1 ? 1 : n * factorial(n - 1);
};

// 2. Передача самой себя как параметра (без накладных расходов):
auto factorial2 = [](auto self, int n) -> int {
    return n <= 1 ? 1 : n * self(self, n - 1);
};
std::cout << factorial2(factorial2, 5) << "\n";  // 120

// 3. C++23: deducing this
auto factorial3 = [](this auto self, int n) -> int {
    return n <= 1 ? 1 : n * self(n - 1);
};
\end{lstlisting}

\subsubsection{Immediately Invoked Lambda Expression (IILE)}

Лямбду можно вызвать сразу на месте --- удобно для инициализации
сложных \texttt{const}-переменных:
...
// config -- const, инициализирован один раз
\end{lstlisting}

% -------------------------------------------------------
\subsection{Указатели на функции, функторы и лямбды --- сравнение}
% -------------------------------------------------------

\begin{center}
\begin{tabular}{l c c c}
\toprule
\textbf{Свойство} & \textbf{Указатель} & \textbf{Функтор} & \textbf{Лямбда} \\
\midrule
Состояние (захват) & Нет & Да & Да \\
\texttt{inline}-оптимизация & Нет\footnote{Компилятор может, но обычно не инлайнит вызов через указатель.} & Да & Да \\
Передача в C~API & Да & Нет & Только без захвата \\
Тип известен & Да & Да & Нет\footnote{Тип уникален и неименуем, нужен \texttt{auto} или \texttt{std::function}.} \\
\bottomrule
\end{tabular}
\end{center}

% -------------------------------------------------------
\subsection{Type erasure (стирание типа)}
% -------------------------------------------------------

\subsubsection{Проблема}

Мы хотим хранить callable-объекты разных типов в~одном контейнере
или передавать их через единый интерфейс. Но:

\begin{itemize}
    \item Указатели на функции не хранят состояние.
    \item Каждая лямбда --- уникальный тип, нельзя положить в~один \texttt{std::vector}.
    \item Шаблоны --- compile-time решение, а нам нужен runtime полиморфизм.
\end{itemize}

\begin{lstlisting}
auto f = [x=1](int a) { return a + x; };
auto g = [y=2](int a) { return a * y; };
// decltype(f) != decltype(g)

// std::vector<???> funcs = {f, g};  // Какой тип указать?
\end{lstlisting}

\subsubsection{Идея type erasure}

\textbf{Type erasure} --- техника, позволяющая <<стереть>> конкретный тип объекта,
сохранив возможность вызывать его операции через единый интерфейс.
Это runtime-полиморфизм \textbf{без} наследования пользователем от базового класса.

Концептуально type erasure состоит из трёх компонентов:

\begin{enumerate}
    \item \textbf{Внешний класс} (wrapper) --- то, с чем работает пользователь.
          Имеет value-семантику (копируется, перемещается).
    \item \textbf{Внутренний абстрактный интерфейс} --- определяет набор операций.
    \item \textbf{Шаблонная реализация} --- конкретная обёртка вокруг
          пользовательского типа, реализующая интерфейс.
\end{enumerate}

\subsubsection{Пример: собственный \texttt{AnyCallable}}

Реализуем упрощённый аналог \texttt{std::function<int(int)>}:
...
class AnyCallable {
    ...
    // Шаг 2: шаблонная реализация для любого callable
    template<typename F>
    ...
    // Шаг 3: хранение через указатель на базу
    std::unique_ptr<Concept> ptr_;

public:
    // Шаблонный конструктор -- принимает что угодно callable
    template<typename F>
    AnyCallable(F f) : ptr_(std::make_unique<Model<F>>(std::move(f))) {}

    // Копирование (через clone)
    AnyCallable(const AnyCallable& other) : ptr_(other.ptr_->clone()) {}
...
int main() {
    AnyCallable f = [x=1](int a) { return a + x; };
    AnyCallable g = [y=2](int a) { return a * y; };

    std::vector<AnyCallable> funcs = {f, g};
    for (auto& func : funcs) {
        std::cout << func(10) << " ";  // 11 20
    }
}
\end{lstlisting}

\subsubsection{Как это работает (диаграмма)}

\begin{verbatim}
AnyCallable                     Concept (abstract)
+-------------------+          +------------------+
| unique_ptr<Concept> ptr_ --> | virtual call()   |
+-------------------+          | virtual clone()  |
                               +------------------+
                                    ^         ^
                                   /           \
...
Ключевая идея: шаблонный конструктор \texttt{AnyCallable(F f)} <<стирает>> тип
\texttt{F}, оборачивая его в~\texttt{Model<F>}, а~хранит указатель на~нешаблонный
\texttt{Concept}.

\subsubsection{Type erasure в стандартной библиотеке}

Type erasure --- не экзотика. Стандартная библиотека активно его использует:

\begin{center}
\begin{tabular}{l l}
\toprule
\textbf{Класс} & \textbf{Что стирает} \\
\midrule
\texttt{std::function<R(Args...)>} & Любой callable с подходящей сигнатурой \\
\texttt{std::any} (C++17)          & Любой копируемый тип \\
\texttt{std::move\_only\_function} (C++23) & Любой move-only callable \\
\texttt{std::format\_args}         & Аргументы форматирования \\
\bottomrule
\end{tabular}
\end{center}
...
\subsubsection{Что это}

\texttt{std::function<R(Args...)>} --- обёртка из \texttt{<functional>},
которая может хранить \textbf{любой} вызываемый объект с~подходящей сигнатурой:

\begin{itemize}
    \item обычную функцию;
    \item лямбду (в~том числе с~захватом);
    \item функтор (объект с~\texttt{operator()});
    \item указатель на метод (через \texttt{std::bind} или лямбду);
    \item результат \texttt{std::bind}.
\end{itemize}
...
int main() {
    // Хранит обычную функцию
    std::function<int(int, int)> f1 = add;
    std::cout << f1(2, 3) << "\n";       // 5

    // Хранит лямбду с захватом
    int offset = 10;
    std::function<int(int)> f2 = [offset](int x) { return x + offset; };
    std::cout << f2(5) << "\n";          // 15
...
\texttt{std::function} может быть пустой (не содержать callable):

\begin{lstlisting}
std::function<void()> f;       // пустая
if (f) {                        // проверка на непустоту
    f();
} else {
    std::cout << "empty!\n";   // сюда попадём
}
...
f = nullptr;                   // снова пустая
// f();                        // выбросит std::bad_function_call!
\end{lstlisting}

\subsubsection{Накладные расходы \texttt{std::function}}

\texttt{std::function} использует type erasure внутри, что влечёт расходы:

\begin{enumerate}
    \item \textbf{Динамическая аллокация.} Если callable не помещается в~SBO
          (Small Buffer Optimization, обычно 16--32~байт), выделяется память в~куче.
    \item \textbf{Виртуальный вызов.} Каждый \texttt{operator()} --- это косвенный
          вызов через vtable (не инлайнится).
    \item \textbf{Копирование.} При копировании \texttt{std::function} копируется
          и~хранимый callable.
\end{enumerate}

\textbf{Когда это важно:}

\begin{lstlisting}
// Плохо: std::function в hot loop
void process(const std::vector<int>& data,
             std::function<int(int)> transform) {  // type erasure
    for (int x : data)
        result.push_back(transform(x));  // косвенный вызов на каждой итерации
}

// Лучше: шаблонный параметр (zero-cost)
template<typename F>
void process(const std::vector<int>& data, F transform) {
    for (int x : data)
        result.push_back(transform(x));  // прямой вызов, инлайнится
}
\end{lstlisting}

\textbf{Правило:} используйте \texttt{std::function}, когда вам \textbf{нужно хранить}
callable (в~поле класса, в~контейнере, в~callback). Для \textbf{передачи}
callable в~функцию предпочитайте шаблоны.

\subsubsection{Хранение указателей на методы}
...
struct Greeter {
    ...
    void hello_n(int n) const {
        for (int i = 0; i < n; ++i)
            std::cout << "Hello, " << name << "!\n";
    }
};

// Метод без аргументов: первый аргумент -- объект
std::function<void(const Greeter&)> f = &Greeter::hello;
Greeter g{"Alice"};
f(g);  // "Hello, Alice!"

// Метод с аргументами:
std::function<void(const Greeter&, int)> f2 = &Greeter::hello_n;
f2(g, 3);  // три раза "Hello, Alice!"

// Привязка объекта через лямбду:
Greeter bob{"Bob"};
std::function<void()> f3 = [&bob]() { bob.hello(); };
f3();  // "Hello, Bob!"
\end{lstlisting}

\subsubsection{\texttt{std::move\_only\_function} (C++23)}

\texttt{std::function} требует, чтобы хранимый callable был \textbf{копируемым}.
Это не работает с~\texttt{unique\_ptr} в~захвате:

\begin{lstlisting}
auto ptr = std::make_unique<int>(42);
// std::function<int()> f = [p = std::move(ptr)]() { return *p; };
// Ошибка: лямбда некопируемая!
\end{lstlisting}

C++23 вводит \texttt{std::move\_only\_function} --- аналог, который
перемещается, но не копируется:
...
std::cout << f() << "\n";  // 42
// auto g = f;  // Ошибка: нельзя копировать
auto g = std::move(f);     // OK: перемещение
\end{lstlisting}

\subsubsection{Пример: система событий (observer pattern)}
...
int main() {
    ...
    bus.subscribe("click", [](const std::string& data) {
    ...
    bus.subscribe("keypress", [](const std::string& data) {
        std::cout << "Key pressed: " << data << "\n";
    });
...
\toprule
\textbf{Механизм} & \textbf{Когда использовать} \\
\midrule
Указатель на функцию & Взаимодействие с~C~API; compile-time dispatch таблицы \\
\addlinespace
Указатель на метод & Обобщённый вызов методов; reflection-подобные паттерны \\
\addlinespace
Указатель на поле & Обобщённый доступ к~полям; сортировка по полю \\
\addlinespace
Лямбда + \texttt{auto}/шаблон & Передача логики в~алгоритмы; zero-cost абстракция \\
\addlinespace
\texttt{std::function} & Хранение callback'ов; runtime полиморфизм callable \\
\addlinespace
Type erasure (свой) & Когда нужен runtime полиморфизм без навязывания базового класса \\
\bottomrule
\end{tabular}
\end{center}
\section{Move-семантика в C++: value categories, rvalue references, perfect forwarding}

\subsection{Value Categories (Категории значений)}

Главное заблуждение относительно move-семантики заключается в том, что категории
\textbf{lvalue} и \textbf{rvalue} --- это свойства объектов. На самом деле --- это
\textbf{синтаксические свойства выражений} (expressions). Один и тот же объект в разных
строчках кода может быть представлен выражениями разных категорий.

Иерархия категорий выглядит следующим образом:
...
\midrule
\texttt{expression} & \texttt{glvalue}, \texttt{rvalue} \\
\texttt{glvalue} & \texttt{lvalue}, \texttt{xvalue} \\
\texttt{rvalue} & \texttt{xvalue}, \texttt{prvalue} \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection{Lvalue (Long-lived)}

Выражения, имеющие имя, адрес в памяти и устойчивую сущность:

\begin{itemize}
  \item Любой идентификатор (имя переменной) --- всегда \texttt{lvalue}.
  \item Результаты присваивания (\texttt{=}, \texttt{+=}), префиксный инкремент/декремент
        (\texttt{++x}), результат разыменования (\texttt{*p}).
  \item Результат функции, возвращающей \texttt{lvalue}-ссылку (\texttt{T\&}).
\end{itemize}

\subsubsection{Rvalue (Short-lived)}

Временные значения, которые будут уничтожены сразу после использования:

\begin{itemize}
  \item Литералы (кроме строковых), результат арифметических операций (\texttt{a + b}),
        постфиксный инкремент/декремент (\texttt{x++}).
  \item Результат функции, возвращающей тип по значению (\texttt{T}) или
        \texttt{rvalue}-ссылку (\texttt{T\&\&}).
  \item Результат приведения типа (\textbf{Cast}) к типу, не являющемуся
        \texttt{lvalue}-ссылкой.
\end{itemize}

\subsubsection{Тернарный оператор и оператор запятая}

Результат тернарного оператора является \texttt{lvalue} только если оба его
подвыражения --- \texttt{lvalue}. Оператор запятая принимает категорию своего
\textbf{правого} операнда.

% -------------------------------------------------------
\subsection{RValue Reference (Rvalue-ссылки)}

Это новый вид ссылок (\texttt{T\&\&}), введённый для реализации семантики перемещения.

\begin{itemize}
  \item \textbf{Инициализация:} \texttt{rvalue}-ссылку можно инициализировать только
        \texttt{rvalue}-выражениями. Попытка привязать её к \texttt{lvalue} вызовет
        ошибку компиляции.
  \item \textbf{Свойство имени:} если \texttt{rvalue}-ссылке дано имя (например,
        \texttt{int\&\& x = 5;}), то само выражение \texttt{x} является
        \texttt{lvalue}, так как это идентификатор.
  \item \textbf{Константность:} нельзя привязать неконстантную ссылку к константному
        объекту --- обычные правила константности сохраняются.
\end{itemize}

\begin{lstlisting}
int&& r = 42;    // OK: привязываем к rvalue
// int&& r2 = r; // Ошибка: r — lvalue (у него есть имя)

const int&& cr = 10; // OK
// int&& r3 = cr;    // Ошибка: нельзя отбросить const
\end{lstlisting}

% -------------------------------------------------------
\subsection{Move Semantics (Семантика перемещения)}

Семантика перемещения позволяет «забирать» ресурсы у временных объектов вместо их
копирования, что критически важно для производительности.

\subsubsection{std::move}

\texttt{std::move} \textbf{ничего не перемещает физически} и не генерирует процессорных
инструкций. Это инструмент для компилятора, преобразующий \texttt{lvalue} в
\texttt{rvalue}.

\textbf{Реализация \texttt{std::move}:}
...
\begin{itemize}
  \item Принимает аргумент по \textbf{универсальной ссылке} (\texttt{T\&\&}), чтобы
        работать с любыми категориями значений.
  \item Использует \texttt{std::remove\_reference\_t<T>}, чтобы получить чистый тип
        без ссылок.
  \item Выполняет \texttt{static\_cast} к типу \texttt{чистый\_тип\&\&}.
\end{itemize}

Результат: выражение \texttt{std::move(x)} всегда является \texttt{rvalue}, что
заставляет компилятор выбирать \textbf{перемещающий конструктор} или
\textbf{оператор присваивания перемещением}.

\begin{lstlisting}
std::vector<int> a = {1, 2, 3};
std::vector<int> b = std::move(a); // a теперь в "valid but unspecified" состоянии
\end{lstlisting}

% -------------------------------------------------------
\subsection{Perfect Forwarding (Идеальная передача)}

Проблема идеальной передачи возникает в функциях-обёртках (например,
\texttt{emplace\_back}), которые должны передать аргументы дальше, сохранив их
оригинальную категорию значения.

\subsubsection{Универсальные ссылки (Universal References)}

Если параметр шаблона функции имеет вид \texttt{T\&\&}, он может принимать и
\texttt{lvalue}, и \texttt{rvalue}:

\begin{lstlisting}
template<typename T>
void wrapper(T&& arg) {
    // arg может быть lvalue-ссылкой или rvalue-ссылкой в зависимости от T
}
\end{lstlisting}

\subsubsection{Сворачивание ссылок (Reference Collapsing)}

При выводе типов шаблонов компилятор «схлопывает» ссылки по следующим правилам:

\begin{center}
\begin{tabular}{l l l}
\toprule
\textbf{T выводится как} & \textbf{Параметр} & \textbf{Результат} \\
\midrule
\texttt{X\&}  & \texttt{T\&\&} & \texttt{X\& \& \&} $\to$ \texttt{X\&} \\
\texttt{X\&}  & \texttt{T\&\&} & \texttt{X\& \&\&} $\to$ \texttt{X\&} \\
\texttt{X\&\&} & \texttt{T\&\&} & \texttt{X\&\& \&} $\to$ \texttt{X\&} \\
\texttt{X\&\&} & \texttt{T\&\&} & \texttt{X\&\& \&\&} $\to$ \texttt{X\&\&} \\
\bottomrule
...
Правило одной строкой: \textbf{только два rvalue дают rvalue}, любое \texttt{\&}
«заражает» результат.

\subsubsection{std::forward}

\texttt{std::forward} необходим, чтобы внутри функции превратить аргумент обратно в
\texttt{rvalue}, если он изначально был таковым (поскольку внутри функции у него есть
имя и он стал \texttt{lvalue}):
...
\begin{lstlisting}
// Реализация std::forward (упрощённо):
template<typename T>
...
% -------------------------------------------------------
\subsection{Variadic Templates (Шаблоны с переменным количеством аргументов)}

В контексте perfect forwarding шаблоны с переменным числом аргументов позволяют
принять \textbf{произвольное число аргументов любых категорий} и передать их дальше
без потери информации о категориях:

\begin{lstlisting}
template<typename... Args>
void emplace_back_impl(Args&&... args) {
    // каждый аргумент в пакете может иметь свою категорию значения
    construct(std::forward<Args>(args)...);
}
\end{lstlisting}

\begin{itemize}
  \item \texttt{Args\&\&... args} --- пакет универсальных ссылок; каждый элемент
        независимо выводит свою категорию.
  \item \texttt{std::forward<Args>(args)...} --- раскрывает пакет, применяя
        \texttt{forward} к каждому элементу отдельно.
  \item Такой подход используется в \texttt{std::vector::emplace\_back},
        \texttt{std::make\_unique}, \texttt{std::make\_shared} и других фабричных
        функциях стандартной библиотеки.
\end{itemize}

\begin{lstlisting}
// Пример: своя фабрика объектов с perfect forwarding
template<typename T, typename... Args>
T* create(Args&&... args) {
...
auto* p = create<Point>(1, 2); // конструируется напрямую, без лишних копий
\end{lstlisting}

\input{parts/05_variadic_metaprog.tex}
